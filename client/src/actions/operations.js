...
export const createNewBatch = batchstate => ({type: ADD_BATCH, payload: batchstate})

export const createNewStudent = studentstate => ({type: ADD_STUDENT, payload: studentstate})

export const removeStudent = id => ({type: DELETE_STUDENT, payload: id})
...
export const getBatches = () => (dispatch, getState) => {
  ...
  request
    ...
    .then(result => dispatch({type: UPDATE_BATCHES, payload: result.body}))
...
export const getStudentsByBatch = (batchid) => (dispatch, getState) => {
  ...
  request
    ...
    .set('Authorization', `Bearer ${jwt}`)
    .then(result => dispatch({type: UPDATE_STUDENTS, payload: result.body}))
    .catch(err => console.error(err))
}
...
export const getRandomStudent = (randomstudent) => {
  return {type: GET_RANDOM_STUDENT, payload: {
      randomstudent
    }}
}
...
export const getCurrentStudent = (id) => (dispatch, getState) => {
  ...
  request
    ...
    .set('Authorization', `Bearer ${jwt}`)
    .then(result => dispatch({type: GET_STUDENT, payload: result.body}))
    .catch(err => console.error(err))
}